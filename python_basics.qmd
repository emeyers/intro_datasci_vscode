...
Comparison operators are used to compare values and produce Boolean results. For example, we can assess whether one number is greater than another number which will return a Boolean:
...
Is the string `"99"` equal to the integer `99` in Python? Try using the equality operator (`==`) to compare them and see what result you get.
...
As we can see, these are not equal, once again showing strings and integers are not the same thing. 
