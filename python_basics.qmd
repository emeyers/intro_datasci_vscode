...
For example, if we multiply integers that are too long, we can get a `ValueError` which indicates that Python is running into problems representing an integer this large.
...
# There is a limited precision to floating point numbers (the last digits are truncated)
...
A **dictionary** is an unordered collection of items where each item is a key-value pair. Dictionaries are mutable, meaning you can change them after they are created. They are useful for storing data in a way that allows you to quickly look up values based on a specific key.

Keys in a dictionary must be unique and are typically strings or numbers. Values can be of any data type, including numbers, strings, lists, or even other dictionaries.

You can create a dictionary by placing key-value pairs inside curly braces `{}`, with each key separated from its value by a colon `:`, and each pair separated by a comma.
...
You can add a new key-value pair to a dictionary or modify an existing one by assigning a value to a key:
...
student_info = {
    ...
    "courses": ["CS101", "Math202", "Physics103"]
}
...
student_info["courses"].append("CS205") # Using the append() method for lists
...
In Python, a **sequence** is an ordered collection of items. This means that the items are stored in a specific order, and you can access them based on their position (index). Common built-in sequence types include strings, lists, and tuples. Many operations work consistently across these sequence types.
...
5.  **Repetition (`*` operator)**: Repeating a sequence a certain number of times.
    ...
    repeated_string = "Ha" * 4
    print(repeated_string) # Output: 'HaHaHaHa'
    ```
...
```
The code evaluates to `True`. In Python, `True` is equivalent to the integer `1` and `False` is equivalent to `0` in arithmetic contexts. So, `True + True + True` is the same as `1 + 1 + 1`, which is `3`. Then, `3 == 3` is `True`.
:::
...
String comparison in Python is done lexicographically (alphabetical order). This means Python compares strings character by character based on their Unicode values (which for English letters correspond to their alphabetical order).
...
```{python}
print("cat" > "car")  # True, because 't' comes after 'r'
print("Zebra" > "apple") # False, because uppercase 'Z' comes before lowercase 'a' in Unicode order.
...
3. Print the value of each result variable.
:::
...
```
The quotient will be a float (`3.75`) because division `/` always results in a float.
:::
...
You are given a string: `raw_data = "  Product_ID:12345, ProductName:SuperWidget, Price:0050.75  "`
