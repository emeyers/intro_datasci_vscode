...
To illustrate this, let's calculate the value of 3 squared (8caret2) which hopefully you remember is equal to the value of 64. In Python, if we want to take a value `x` to the power `y` (i.e., to calculate xcarety) we use the syntax `x**y`. So, if we wanted to calculate 8care2 we would write the following Python code: 
...
An *assignment statement* is a line of code that is used to store a value in a named **variable**. We can then refer back to this variable name to retrieve the value we have stored. For example, the following code assigns the value `10` to the variable `a`:
