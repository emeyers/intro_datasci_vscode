...
ax.text(1.40, 1.62, "by position", fontsize=8, style="italic", color=highlight)
...
box_w, box_h = 1.50, 0.34

# The two values in a row are drawn at fixed offsets inside the box rather than
# as one centred string, so that the carrier codes line up under each other and
# so do the numbers, however many digits each one has. The offsets are far
# enough apart to leave a gap under the longest heading, "mean_delay".
key_dx, value_dx = 0.36, 1.12


def cell(x, y, key, value, colour, width=box_w):
    ax.add_patch(patches.FancyBboxPatch(
        (x, y), width, box_h, boxstyle="round,pad=0.01", fc=colour, ec="#9ab", lw=0.9))
    ax.text(x + key_dx, y + box_h / 2, key, ha="center", va="center",
            fontsize=8, family="monospace")
    ax.text(x + value_dx, y + box_h / 2, value, ha="center", va="center",
            fontsize=8, family="monospace")


def headings(x, key, value):
    ax.text(x + key_dx, head_y, key, ha="center", fontsize=7.5,
            family="monospace", color="0.35")
    ax.text(x + value_dx, head_y, value, ha="center", fontsize=7.5,
            family="monospace", color="0.35")


# Column headings clear the tallest first row of the three stages, which is the
# split column's, and the stage names sit above those. Both are measured off the
# cells so they follow if the row geometry changes.
top = 2.30
split_top = top + 0.06
head_y = split_top + box_h + 0.22
title_y = head_y + 0.32

# stage 1: the original DataFrame, coloured by the group each row belongs to
for i, (carrier, delay) in enumerate(rows):
    y = top - i * 0.40
    cell(0.10, y, carrier, str(delay), colour_of[carrier])
headings(0.10, "carrier", "dep_delay")
ax.text(0.10 + box_w / 2, title_y, "the DataFrame", ha="center", fontsize=9,
        weight="bold")
...
for carrier, delays, _, colour in groups:
    block_top = y_cursor
    for d in delays:
        cell(x_split, y_cursor, carrier, str(d), colour)
        y_cursor -= 0.40
    # centre of this block, taken from the rows just drawn rather than guessed
    block_centres.append((block_top + (y_cursor + 0.40)) / 2)
    y_cursor -= 0.18
headings(x_split, "carrier", "dep_delay")
ax.text(x_split + box_w / 2, title_y, "split", ha="center", fontsize=9,
        weight="bold")
...
for (carrier, _, mean, colour), cy in zip(groups, block_centres):
    cell(x_out, cy, carrier, f"{mean:.1f}", colour)
headings(x_out, "carrier", "mean_delay")
ax.text(x_out + box_w / 2, title_y, "apply, then combine", ha="center",
...
ax.text(x_out + box_w / 2, min(block_centres) - pad - 0.22, "one result DataFrame",
        ha="center", fontsize=8, style="italic", color="0.3")

# Arrows between the stages, each naming the pandas operation that does the step
# above the detail it is given. The second stops short of the dashed box.
for x0, x1, operation, detail in ((1.72, 2.55, "group by", "by carrier"),
                                  (4.27, 5.23, "aggregate", "mean")):
    ax.annotate("", xy=(x1, 1.30), xytext=(x0, 1.30),
                arrowprops=dict(arrowstyle="->", color="0.35", lw=1.4))
    ax.text((x0 + x1) / 2, 1.60, operation, ha="center", fontsize=8.5,
            weight="bold", color="0.25")
    ax.text((x0 + x1) / 2, 1.40, detail, ha="center", fontsize=8,
