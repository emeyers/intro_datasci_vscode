...
These two ideas, indexing with a row and a column, and slicing along either axis, are exactly what we'll use to work with images for the rest of this section.
...
The shape `(512, 512)` tells us the photo is 512 pixels tall and 512 pixels wide, with a single brightness value at each position, just like the small example above. The data type `uint8` means each value is a whole number from 0 to 255, where 0 is no intensity (black) and 255 is full intensity (white).
